...
void lcdInitHW(void)
{
	...
	// inttialize LCD data lines to pull-up
	#ifdef LCD_DATA_4BIT
...
void lcdBusyWait(void)
{
	...
	#ifdef LCD_DATA_4BIT
		outb(LCD_DATA_DDR, inb(LCD_DATA_DDR)&0x0F);	// set data I/O lines to input (4bit)
		outb(LCD_DATA_POUT, inb(LCD_DATA_POUT)|0xF0); // set pull-ups on (4bit)
	#else
		outb(LCD_DATA_DDR, 0x00);	// set data I/O lines to input (8bit)
		outb(LCD_DATA_POUT, 0xFF);	// set pull-ups to on (8bit)
	#endif
	...
	cbi(LCD_CTRL_PORT, LCD_CTRL_E);		// clear "E" line
	// leave data lines in input mode so they can be most easily used for other purpose
#else
	// memory bus read
...
// write the control byte to the display controller
	...
	cbi(LCD_CTRL_PORT, LCD_CTRL_RW);	// set R/W to "write"
		...
		LCD_DELAY;					// wait
		LCD_DELAY;					// wait
		cbi(LCD_CTRL_PORT, LCE_CTRL_E); // clear "E" line
	#endif
...
uint8_t lcdControlRead(void)
{
	...
	sbi(LCD_CTRL_PORT, LCD_CTRL_RW);	// set R/W to "read"
		...
		cbi(LCD_CTRL_PORT, LCD_CTRL_E);	// clear "E" line
	#endif
	// leave data line in input mode so they can be most easily used for other purposes
#else
	//sbi(MCUCR, SRW);		// enable RAM waitstate
	lcdBusyWait();			// wait until LCD not busy
...
// write a data byte to the display
	...
	// memory bus write
	//sbi(MCUCR, SRW);		// enable RAM waitstate
	lcdBusWait();			// wait until LCD not busy
...
// read a data byte from the display
	...
	sbi(LCD_CTRL_PORT, LCD_CTRL_RW);	// set R/W to "read"
		...
		LCD_DELAY;					// wait
		LCD_DELAY;					// wait
		data |= inb(LCD_DATA_PIN)>>4; // input data, low 4bits
		cbi(LCD_CTRL_PORT, LCD_CTRL_E); // clear "E" line
	#else
		// 8 bit read
		sbi(LCD_CTRL_PORT, LCD_CTRL_E);	// set "E" line
