...
//
// About Timers
// The Atmel AVR-series processors each contain at least one hardware
// timer/counter. Many of the processors contain 2 or 3 timers. Generally
// speaking, a timer is a hardware counter inside the processor which counts
// at a rate related to main CPU clock frequency. Because the counter value
// increasing (counting up) at a precise rate, we can use it as a timer to 
// create or measure precise delays, schedule events, or generate signals of
// a certain frequency or pulse-width
//
...
// count value of 256 for 8bits or 65536 for 16bits. A prescaler is available
// for each timer, and the prescaler allows you to pre-divide the main CPU
// clock rate down to a slower speed before feeding it to the counting input
// of a timer. For example, if the CPU clock frequency is 3.69MHz, and Timer0's
// prescaler is set to divide-by-8, then Timer0 will "tic" at 
// 3690000/8 = 461250Hz. Because Timer0 is an 8bit timer, it will count to
// 256 in just 256/461250Hz = 0.555ms. In fact, when it hits 255, it will 
// overflow and start again at zero. In this case, Timer0 will overflow
// 461250/256 = 1801.76 times per second.
//
// Timer0 can be used a number of ways simultaneously. First, the value of
// the timer can be read by accessing the CPU register TCNT0. We could,
// for example, figure out how long it takes to execute a C command by recording
// the value of TCNT0 before and after execution, then subtract (after-before)
//  = time elased. Or we can enable the overflow interrupt which goes off
//  every time T0 overflows and count out longer delays (multiple overflows),
//  or execute a special periodic function at every overflow.
//
//  The other timers (Timer1 and Timer2) offer all the abilities of Timer0 and
//  many more features. Both T1 and T2 can operate as general-purpose timers,
//  but T1 has special hardware allowing it to generate PWM signals, while
//  T2 is specially designed to help count out real time (like hours, minutes,
//  seconds). See the Timer/Counter section of the processor datasheet for
//  more info.
...
// 		to a timer interrupt. "Attaching" one of your own fucntions to a timer
// 		interrupt means that it will be called whenever that interrupt happens.
// 		Using attach is better than rewriting the actural ISR() function
...
//
// 		timerAttach cause the myOverflowFunction() to be attached, and therefore
...
// bitRes indicates the period/resolution to use for PWM output in timer bits.
// 		  Must be either 8, 9, or 10 bits corresponding to PWM periods of 256, 
// 		  512, or 1024 timer tics.
void timer1PWMInit(uint8_t bitRes);

// Enter PWM Mode on timer1 with a specific top-count value.
// topcount indicates the desired PWM period in timer tics.
// Can be a number between 1 and 65535 (16-bit).
void timer1PWMInitCR(uint16_t topcount);
