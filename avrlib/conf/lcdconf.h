...
// 		LCD to the CPU's memry bus. The CONTROL and DATA registers of the LCD
...
// 		use an external memory bus for other purpose (like accesing memory).
//
// 	LCD_PORT_INTERFACE:
...
// 		be accessed in 8-bit or 4-bit mode. In 8-bit mode, transfers are faster.
// 		In 4-bit mode, only I/O port pins 4-7 are needed for data lines, but
// 		LCD access is slower. In either mode, three additional port pins are
// 		required for the LCD interface control lines (RS, R/W, and E).

// Enable one of the folling interfaces to your LCD
...
// if you chose the LCD_PORT_INTERFACE:
		...
		// port you will user for data lines
		#define LCD_DATA_POUT	PORTD
		#define LCD_DATA_PIN	PIND
		#define	LCD_DATA_DDR	DDRD
		// access mode you will use (default is 8bit unless 4bit is selectd)
		#define LCD_DATA_4BIT
	#endif
#endif

// if you choose the LCD_MEMORY_INTERFACE:
...
#define LCD_LINE_LENGTH		20	// line length (in charaters)
...
// LCD_DELAY should be scaled to take at least half that time (500ns).
// Each NOP takes 1 CPU clock cycle to execute. Thus, at 4MHz, you should
// use at lest 2 NOPs, at 8MHz at least 4 NOPs, etc.
