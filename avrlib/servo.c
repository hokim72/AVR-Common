...
void servoInit(void)
{
	...
	for (channel=0; channel<SERVO_NUM_CHANNELS; channel++)
	{
		...
		// ServoChannels[channel].port = (uint16_t)&SERVO_DEFAULT_PORT;
		ServoChannels[channel].pin = (1 << channel);
		// set channel pin to output
		// THIS IS OBSOLETED BY THE DYNAMIC TO PORT, PIN ASSIGNMENTS
		// outb(SERVODDR, inb(SERVODDIR) | (1<<channel));
	}
	// set PosTics
	ServoPosTics = 0;
	// set PeriodTics
...
void servoSetChannelIO(uint8_t channel, uint8_t port, uint8_t pin)
{
	...
	ServoChannels[channel].pin = (1<<(pin&0x7));
}
