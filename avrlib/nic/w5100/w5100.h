...
#define W5100_IR					0x0015 // Interupt Register
...
// Define the address inside the W5100 for the transmit and receive buffers.
...
#define W5100_TX_BUF_SIZE			0x0800 // Tx 2K Buffer Size
#define W5100_RX_BUF_SIZE			0x0800 // Rx 2K Buffer Size

#define W5100_TX_BUF_MASK			0x07FF // Tx 2K Buffer Mask
#define W5100_RX_BUF_MASK			0x07FF // Rx 2K Buffer Mask

#define W5100_TX_BUF_BASE(n)		(W5100_TXBUFADDR+(n*W5100_TX_BUF_SIZE))
#define W5100_RX_BUF_BASE(n)		(W5100_RXBUFADDR+(n*W5100_RX_BUF_SIZE))
...
// The following #defines are OR-masks for creating mode values to write to
// the Mode register. Note that these values are only valid when writing to
// the chips Mode register. These values are NOT to be used with the individual
// socket Mode registers.
#define W5100_MR_SOFTRST			0x80 // soft-reset
#define W5100_MR_PINGBLK			0x10 // block responses to ping request
#define W5100_MR_PPPOE				0x08 // enable PPPoE
#define W5100_MR_AUTOINC			0x02 // address autoincrement (indirect interface ONLY!)
#define W5100_MR_INDINT				0x01 // use indirect interface (parallel interface ONLY!)

// The following #defines are OR-masks for checking bits in the Interrupt Register,
// used by the host MCU to determine the current state of the W5100. Note that
// the values are only valid with writing to the chip's Interrupt register.
// These values are NOT to be used with the individual's socket Interrupt registers.
#define W5100_IR_CONFLICT			0x80 // 1 means ARP request received with this machine's IP
#define W5100_IR_UNREACH			0x40 // 1 means IP addr & port were unreachable (ICMP packet received)
#define W5100_IR_PPPOE				0x20 // 1 means PPPoE connection closed
#define W5100_IR_S3_INT				0x08 // 1 means socket 3 interrupt occurred
#define W5100_IR_S2_INT				0x04 // 1 means socket 2 interrupt occurred
#define W5100_IR_S1_INT				0x02 // 1 means socket 1 interrupt occurred
#define W5100_IR_S0_INT				0x01 // 1 means socket 0 interrupt occurred

// The following #defines are OR-masks for selecting features in the Interrupt Mask Register.
...
#define W5100_IMR_CONFLICT			0x80 // Enable interrupt for ARP request received with this machine's IP
#define W5100_IMR_UNREACH			0x40 // Enable interrupt for IP addr & port were unreachable (ICMP packet received)
#define W5100_IMR_PPPOE				0x20 // Enable interrupt for PPPoE connection closed
#define W5100_IMR_S3_INT			0x08 // Enable interrupt for socket 3 interrupt
#define W5100_IMR_S2_INT			0x04 // Enable interrupt for socket 2 interrupt
#define W5100_IMR_S1_INT			0x02 // Enable interrupt for socket 1 interrupt
#define W5100_IMR_S0_INT			0x01 // Enable interrupt for socket 0 interrupt


// The following #defines are values written to a socket's Mode register to 
// select the protocol and other operting characteristics.
...
// The following #defines are values for selecting commands in the Command registers
...
#define W5100_SKT_IR_TIMEOUT		0x08
#define W5100_SKT_IR_RECV			0x04
#define W5100_SKT_IR_CON			0x01

// The following #defines are values that can be read from the Status registers
// for each of the sockets.
...
// be called at least once, immediately after your target's hardware has been
// configured.
void W51_init(void);

// read a byte from a specific address in the W5100.
// return the byte read from the specified address.
// addr: a 16-bit address of a register in the W5100 chip.
uint8_t W51_read(uint16_t addr);

// write a byte to a specific address in the W5100
// addr: a 16-bit address of a register in the W5100 chip.
// data: a byte of data to send to the W5100 after the address has been set up.
void W51_write(uint16_t addr, uint8_t data);

// This fucntion is being used for copy the data from Receive buffer of the chip to application buffer.
// It calculate the actual physical address where one has to read the data from Receive buffer. Here also take care of the condition that it exceeds the Rx memory upper-bound of socket.
void W51_read_data(uint16_t s, volatile uint8_t* src, volatile uint8_t* dst, uint16_t len);

// for copy the data from application buffer to Transmit buffer of the chip.
// This function is being used for copy the data from application buffer to Transmit buffer of the chip. It calculate the actual physical address where one has to write the data in transmit buffer. Here also takes care of the condition that it exceeds the Tx memory upper-bound of socket.
void W51_write_data(uint8_t s, volatile uint8_t* src, volatile uint8_t* dst, uint16_t len);

// This function is being called by TCP send() and UDP & IP_RAW snedto() function.
// This function reads the Tx write pointer register and after copy the data in buffer update the Tx write pointer register. User should read upper byte first and low byte later to get proper value.
void W51_send_data_processing(uint8_t s, uint8_t* data, uint16_t len);

// This function is being called by TCP recv().
