...
// When you run ServoInit, it automatically assigns each "channel" of servo
// control to be output on the SERVO_DEFAULT_PORT. One "channel" of servo
// control can control one servo and must be assigned signle I/O pin for output.
//
// If you're using all eight channels (SERVO_NUM_CHANNELS = 8), then by default
// the code will use SERVO_DEFAULT_PORT pins 0-7. If you're only using four
// channels, then pins 0-3 will be used by default.
//
// The command servoSetChannelIO(channel, port, pin) allows you to reassign
// the output of any channel to any port and I/O pin you choose. For example,
// if you have an RC servo connected to PORTC, pin 6, and you wish to use
// channel 2 to control it, use:
//
...
// a given servo to your desired position. The position you request must lie
// between the SERVO_MIN and SERVO_MAX limit you defined.

#ifndef SERVOCONF_H
#define SERVOCONF_H

// set number of servo channels (1 to 8)
// 		This is the number of servos you would like to drive. Each "Channel"
// 		can control one servo and by default will map directly to the port pin
// 		of the same number on the SERVO_DEFAULT_PORT. You can change this
// 		default port/pin assignment for a given channel to any port/pin you
// 		like. See the "servoSetChannelIO" function.
