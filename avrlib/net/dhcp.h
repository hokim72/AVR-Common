...
// Configuration Protocol) as described in RFC2131. DHCP allows a network
// device to automatically obtain an IP address and other network configuration
...
#define DHCP_OPT_PARAMREQLIST 55 // Parameter Request List (n OPT codes)
...
// Process incoming DHCP packets from UDP port 68.
// This function is to be called by the stack when a DHCP packet arrives over
// the network. The DHCP packet will be parsed, handled, and a response will be
// generated and sent if needed. When the DHCP process completes, the IP
// addressing will be automactically update. */
...
// This function releases the DHCP assigned address and allows the DHCP
// server to reallocate it.
void dhcpRelease(void);

// Periodic DHCP maintenance.
// This function is to be called once per second and will expire the DHCP lease.
void dhcpTimer(void);
