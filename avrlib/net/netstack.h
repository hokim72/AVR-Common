// This library co-ordinates the various pieces of a typical IP network stack
// into one unit. Included are handling for ARP, ICMP, and IP packets. UDP and
// TCP packets are processed and passed to the user.
//
// This is an example of how to use the various network libraries, and is
// meant to be useful out-of-the-box for most users. However, some users may
// find it restrictive and write their own handlers instead.
// This stack implementation is by no means the only way to use the various
// network libraries.
//
// This is NOT a full-blown TCP/IP stack. It merely handles lower level stack
// functions so that UDP and TCP packets can be sent and received easily.
// End-to-end TCP functionality may be added in a future version. Until then,
// I can recommand using other embedded TCP/IP stacks like Adam Dunkel's uIP.
...
// You may override the default NET_BUFFERSIZE by defining an alternate value
// in global.h
// Network packets larget than NET_BUFFERSIZE will not be accepted.
...
// Users should define this same function in user code (same name and arguments)
// to override this default handler and get access to the received packets.
...
// Users should define this same function in user code (same name and arguments)
// to overrides the default handler and get access to the received packets.
