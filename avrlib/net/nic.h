// This is the software interface standard for network interface hardware as
// used by AVRlib. Drivers for network hardware must implement these functions
// to allow upper network layers to initialize the interface, and send and
// and receive net traffic.
...
// the network interface ready to handle nicSend() and nicPoll() requests.
// For some hardware, this command will take a non-negligible amount of time
// (1-2 seconds).
void nicInit(void);

// Send packet on network interface.
// Function accepts the length (in bytes) of the data to be sent, and a pointer
// to the data. This send command may assume and ethernet-like 802.3 header is
// at the beginning of the packet, and contains the packet addressing information.
// See net.h documentation for ethernet header format.
void nicSend(unsigned int len, unsigned char* packet);

// Check network interface; return next received packet if available.
// Function accepts the maximum allowable packet length (in bytes), and a
// pointer to the received packet buffer. Return value is the length (in bytes)
// of the packet received, or zero if no packet is available.
...
// See net.h documentation for ethernet format.
...
// If the hardware does not provide a MAC address, a software-defined address maybe returned. It may be acceptable to return an address that is less than 48-bits.
